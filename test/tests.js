...
describe("testando estilo normal", () => {
  ...
  test("escrevendo centenas de milhares", () => {
    [
      ...
      assert.equal(numero.porExtenso((i + 1) * 100000), n + " mil")
    );
  });
  ...
  test("Formatando número", () => {
    assert.equal(numero.formataNumero(12523), 12.523);
  });

  //Retorna a extensão do arquivo de um caminho
  const caminho = "C:Projetos/Arquivos/teste.kml";

  test("Testando retorno extensão", () => {
    assert.equal(numero.getExtensao(caminho), "kml");
  });
});
...
describe("testando README.md", () => {
  test("escrevendo exemplos", () => {
    ...
    // Números decimais
    assert.equal(numero.porExtenso(10.5), "dez vírgula cinco décimos");
    assert.equal(
    ...
    assert.equal(
      numero.porExtenso(999_999_999_999_999.9),
...
describe("Issue #4", () => {
  test("escrevendo percentuais com zeros a esquerda", () => {
    assert.equal(
      numero.porExtenso(3.01, numero.estilo.porcentagem),
      "três vírgula um centésimo por cento"
    );
    assert.equal(
      numero.porExtenso(50.05, numero.estilo.porcentagem),
      "cinquenta vírgula cinco centésimos por cento"
    );
    assert.equal(
      numero.porExtenso(1.049, numero.estilo.porcentagem),
